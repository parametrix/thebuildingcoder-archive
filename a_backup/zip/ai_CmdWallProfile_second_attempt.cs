...
namespace WallProfileForBuildingCoder
{
    ...
    public class CmdWallProfile : IExternalCommand
    {
                    ...
                    if (((LocationCurve) wall.Location).Curve is Line)
                    {
                        ...
                        if (isCounterClockwize)
                            foreach (ModelCurve mcurve in curveElements)
                                SetModelCurveColor(mcurve, view, colorRed);
                    }
                    else
                        foreach (var curve in curves.Cast<Curve>())
                        {
                            var modelCurve = creator.CreateModelCurve(curve);
                            if (isCounterClockwize)
                                SetModelCurveColor(modelCurve, view, colorRed);
                        }
                }
        ...
        private void SetModelCurveColor(ModelCurve modelCurve, View view, Color color)
        {
