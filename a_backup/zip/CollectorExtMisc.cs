      ...
      Stream(ArrayList data, ExtensibleStorageField storageField)
      {
         ...
         if (schema != null)
         {
            ...
            if (field.UnitType != UnitType.UT_Undefined)
               dut = m_activeDoc.ProjectUnit.get_FormatOptions(field.UnitType).Units;

            switch( field.ContainerType )
            {
              case ContainerType.Simple:
                {
                  //MethodInfo genericGet = methodInfo.MakeGenericMethod(field.ValueType); // removed by jeremy
                  MethodInfo genericGet = methodInfo.MakeGenericMethod( new Type[] { field.ValueType } ); // added by jeremy
              ...
              case ContainerType.Array:
                {
                  ...
                  //MethodInfo genericGet = methodInfo.MakeGenericMethod(type); // removed by jeremy
                  MethodInfo genericGet = methodInfo.MakeGenericMethod( new Type[] { type } ); // added by jeremy
                  Object[] parameters = new Object[2] { field, dut };
                  value = genericGet.Invoke( entity, parameters );
                  break;
                }
